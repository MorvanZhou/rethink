...
from rethink.models.search.engine_es import ESSearcher, SearchDoc
from . import utils
...
class ESTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncTearDown(self) -> None:
        await self.searcher.drop()
        self.assertFalse(await self.searcher.es.indices.exists(index=self.searcher.index_name))
    ...
    async def test_add(self):
        ...
        self.assertEqual('this is 19 <b class="match term0">doc</b>, 这是第 19 个文档', docs[0].highlights)
    ...
    async def test_batch_add_update_delete(self):
        ...
        time.sleep(0.01)
        await self.searcher.add_batch(uid="uid", docs=[SearchDoc(
            nid=f"nid{i}",
            title=f"title{i}",
            md=f"this is {i} doc, 这是第 {i} 个文档",
